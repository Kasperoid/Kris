...
public:
	...
	void solve_ex() {
		int Sem_num, flag_2 = 0, flag_3 = 0, flag_4 = 0, flag_5 = 0;
		char file_name_otl[STRLEN] = "Otl.txt", file_name_hor[STRLEN] = "Hor.txt", file_name_tro[STRLEN] = "Tro.txt";
		FILE* file;
		FILE* file_otl;
		FILE* file_hor;
		FILE* file_tro;
	...
	sem1:
		...
		input(&Sem_num);
		if (Error_semestr_num(Sem_num) == 1) {
			printf("%s", Error[8]);
			goto sem1;
		}
		fopen_s(&file, file_name, "r+b");
		fopen_s(&file_otl, file_name_otl, "w+b");
		fopen_s(&file_hor, file_name_hor, "w+b");
		fopen_s(&file_tro, file_name_tro, "w+b");
		while (1) {
			a1:
			fread_s(&student, sizeof(student), sizeof(student), 1, file);
			flag_2 = 0;
			...
			flag_4 = 0;
			flag_5 = 0;
			int* buffer = new int[student.S[Sem_num - 1].Pnum];
			if (feof(file)) break;
			if (student.Snum >= Sem_num) {
				for (int i = 0; i < student.S[Sem_num - 1].Pnum; i++) {
					buffer[i] = student.S[Sem_num - 1].p[i].mark;
				}
			}
			for (int i = 0; i < student.S[Sem_num - 1].Pnum; i++) {
				if (buffer[i] == 2) {
			...
			if (flag_3 == 1) {
				fwrite(&student, sizeof(student), 1, file_tro);
				delete[] buffer;
				goto a1;
			}
			if (flag_4 == 1) {
				fwrite(&student, sizeof(student), 1, file_hor);
				delete[] buffer;
				goto a1;
			}
			if (flag_5 == 1) {
				fwrite(&student, sizeof(student), 1, file_otl);
				delete[] buffer;
				goto a1;
			}
		}
		...
		fclose(file_hor);
		fclose(file_tro);
		if (Error_zero(file_name_otl) == -1) {
			printf("Отличников не было найдено\n");
		}
		else {
			fopen_s(&file_otl, file_name_otl, "r+b");
			printf("\nОТЛИЧНИКИ\n");
			print_file(file_otl);
			fclose(file_otl);
		}
		if (Error_zero(file_name_hor) == -1) {
			printf("Хорошистов не было найдено\n");
		}
		else {
			fopen_s(&file_hor, file_name_hor, "r+b");
			printf("\nХОРОШИСТЫ\n");
			print_file(file_hor);
			fclose(file_hor);
		}
		if (Error_zero(file_name_tro) == -1) {
			printf("Троечников не было найдено\n");
		}
		else {
			fopen_s(&file_tro, file_name_tro, "r+b");
			printf("\nТРОЕЧНИКИ\n");
			print_file(file_tro);
			fclose(file_tro);
		}
